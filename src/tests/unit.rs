...
fn test_multi_statement_multiple_violations_are_all_reported() {
    let violations =
        analyze_missing_where_operations("UPDATE users SET active = false; DELETE FROM sessions;");
...
fn test_comments_and_strings_do_not_fake_where() {
    let violations = analyze_missing_where_operations("UPDATE users SET note = 'WHERE id = 1';");
    assert_eq!(violations, vec![Operation::Update]);
